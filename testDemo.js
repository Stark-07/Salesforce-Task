...
import fetchLookupData from '@salesforce/apex/AppointmentController.fetchLookupData';
import fetchDefaultRecord from '@salesforce/apex/AppointmentController.fetchDefaultRecord';

const DELAY = 300; // dealy apex callout timing in miliseconds  

export default class test_Demo extends LightningElement {
    // public properties with initial default values 
    @api label = 'custom lookup label';
    @api placeholder = 'search...'; 
    @api iconName = 'standard:account';
    @api sObjectApiName = 'Appointment_Slots__c';
    @api defaultRecordId = '';
...
@api startTime; // This will receive Start_Time__c from the parent component or Apex

    // Getter to format the date-time
    get formattedStartTime() {
        if (this.selectedRecord.Start_Time__c) {
            const options = {
                hour: '2-digit',
                minute: '2-digit',
                second: '2-digit',
            ...
            return new Intl.DateTimeFormat('en-US', options).format(new Date(this.selectedRecord.Start_Time__c));
        }
        return '';
    }
}